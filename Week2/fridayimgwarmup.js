...
then create two additional functions that run off the data returened off the first function

so they take function1 to then find total sales and gross profit.

some built in methods

object.keys(); method- prints out array

object.hasOwnProperty();

object.length

stretch goal is to just figure it out all by itself.*/


//start with a function that runs everything through it in an array
var a= date;
var b= candy;
var c= prices;
var d= number;

function storebook(storeNum) {
      for (i=0; i < store1[i].length; i++)
      
  }
}
//move into, depending on what store # (through switch), depending on store number,  run through if else sub function that normalizes the store by varaibles.

//after new value of store has been established in a new object, run function on code to check for values.
